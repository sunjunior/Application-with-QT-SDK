...
static bool createConnection()
{
    ...
    db.setHostName("www.huawei.com");
    db.setDatabaseName("data.db");
    db.setUserName("root");
    db.setPassword("huawei");
    if (!db.open())
    {
        //提示出错
        return false;
    ...
    // 创建用户密码表
    query.exec("create table login(usrname varchar primary key, password varchar)");
    query.exec("insert into login values('root','huawei')");
